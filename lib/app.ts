...
export class App {
    ...
    public static placeLtR(word: string, x: number, y: number, table: Table, affix?: string): boolean {
        return App.placeLetters(word, x, y, table, 1, 0, affix);
    }
    ...
    public static placeTtB(word: string, x: number, y: number, table: Table, affix?: string): boolean {
        return App.placeLetters(word, x, y, table, 0, 1, affix);
    }
    ...
    public static placeDDR(word: string, x: number, y: number, table: Table, affix?: string): boolean {
        return App.placeLetters(word, x, y, table, 1, 1, affix);
    }
    ...
    public static listAlgorithms(length: number, width: number, height: number): PlacementFunction[] {
        ...
        if (canHorizontal) {
            arrangements.push(App.placeLtR);
            arrangements.push(App.placeRtL);
            if (canVertical) {
                arrangements.push(App.placeDDL);
                arrangements.push(App.placeDDR);
                arrangements.push(App.placeDUL);
                arrangements.push(App.placeDUR);
            }
        }
        if (canVertical) {
            arrangements.push(App.placeTtB);
            arrangements.push(App.placeBtT);
        }
        return arrangements;
    }

    public static placeWord(word: string, algs: PlacementFunction[], table: Table): boolean {
        let x = App.random(0, table.columns);
        let y = App.random(0, table.rows);
        let alg = App.random(0, algs.length);
    ...
    public static placeWordPart(word: string, algs: PlacementFunction[], table: Table, minLength: number, maxLength: number): number {
        let segmentLength = App.random(minLength, maxLength);
        let segmentStart = App.random(0, word.length - segmentLength);
        let segment = word.substr(segmentStart, segmentLength);
        if (App.placeWord(segment, algs, table)) {
    ...
    public static findWord(word: string, x: number, y: number, table: Table): number {
        ...
        for (let xDir = -1; xDir <= 1; xDir++) {
            for (let yDir = -1; yDir <= 1; yDir++) {
                if (App.findWordInDirection(word, x, y, xDir, yDir, table)) {
                    count++;
                }
            }
        }
    ...
    generate() {
        ...
        let arrangements = App.listAlgorithms(this.word.length, this.width, this.height);
        for (let i = 0; i < this.retryCount; i++) {
            if (App.placeWord(this.word, arrangements, this.data)) {
                break;
            }
        }
        ...
        for (let i = 0; i < attemptMax && remaining > 0; i++) {
            remaining -= App.placeWordPart(this.word, arrangements, this.data, min, max);
        }
        while (remaining > 0) {
            remaining -= App.placeWordPart(this.word, arrangements, this.data, 1, 1);
        }
        this.count = 0;
        for (let i = 0; i < this.data.rows; i++) {
            for (let j = 0; j < this.data.columns; j++) {
                this.count += App.findWord(this.word, j, i, this.data);
            }
        }
    }
}
